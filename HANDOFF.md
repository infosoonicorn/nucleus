...
**Article tag canonicalisation landed (2026-05-21).** Replaced 121 free-text tags spread across 135 articles with a locked 31-entry canonical vocabulary, ~3–4 tags per service line. `/insights` filter chip cloud now reads as a real filter (each chip lands on 2–8 articles, not a wall of 1-of-1 strings). Card labels everywhere (insights hub, service-page insights row, "More from author", "Related articles", team profile modal, home insights row) now read canonical buckets like `Deal mechanics` / `Internal audit & ICFR` / `Fund structuring`. Added build-time validator `lint:articles:tags` wired into `pnpm lint` chain — locks the vocabulary so any future tag drift fails CI. 10 atomic commits (1 validator + 9 per-service rewrites) plus 1 in-flight fixup for two double-quoted-with-apostrophe tag literals (`Int'l tax`). Pre-existing typecheck failure in untracked `apps/web/src/components/coming-soon.tsx` (framer-motion `Easing` type narrowing) noted; unrelated to this work — needs separate fix when that component is finalised.
